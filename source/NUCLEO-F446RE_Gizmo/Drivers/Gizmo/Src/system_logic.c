...
static UART_OP_STATUS PeripheralAddCMD(peripheral_state* per, UART_COMMANDS new_cmd)
{
...
UART_OP_STATUS UART_Decode(uint8_t* msg_buffer)
{
	...
	switch( *(msg_buffer + CMD_POS_BYTE))
	{
		case COM_SERVO_POS_SET:
		{
			...
			uint8_t servo_line = *(msg_buffer + PER_LINE_BYTE);
			// Servo line id
			if( servo_line == 0x00)
			{
			...
			*(buffer) =  ((uint16_t) *(msg_buffer + PER_LINE_BYTE + 1) << 8) | ((uint16_t) *(msg_buffer + PER_LINE_BYTE + 2) );
			*(buffer + 1) =  ((uint16_t) *(msg_buffer + PER_LINE_BYTE + 3) << 8) | ((uint16_t) *(msg_buffer + PER_LINE_BYTE + 4) );
			*(buffer + 2) = (uint16_t) *(msg_buffer + + PER_LINE_BYTE + 5);
